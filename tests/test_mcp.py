...
def test_hook_context_is_applied_to_every_bridged_client(tmp_path):
    ...
    Replaces the pre-fix contract where only claude-code could inherit.
    Measured consequence of that contract: 1,161 Codex sections with ZERO
    carrying a client_session_id, and 61% of work items never joining to usage.
    """
    server = SentinelMCPServer(store_dir=tmp_path / "state")
...
def test_shipped_instructions_name_the_argument_the_schema_accepts(tmp_path):
    ...
    # The contract names the argument the schema accepts. The surrounding wording
    # is deliberately not pinned -- only the argument name, which is the thing
    # that would ship a call the server rejects.
...
def test_machine_check_name_has_no_private_length_cap(tmp_path):
    ...
    # Trailing whitespace is normalized away by the display rules, so measure a
    # name that is already normalized.
...
def test_machine_check_name_band_is_the_measured_one(tmp_path):
    ...
    # moves whenever the server adds context to the metadata; assert the shape.
