...
def _work_card_markup(s: dict, pal: dict[str, str], *, expanded: bool = False) -> str:
    ...
    if expanded:
        ...
        col = 15
        # The COVERAGE cell is padded on its VISIBLE text (escaping adds markup
        # characters that take no columns) and always keeps at least one space
        # before CHECKS — a 13-character coverage word used to butt straight
        # into the count ("not gradeable1/2").
        cov_cell = cov.ljust(max(col - 2, len(cov) + 1))
        grid = (f"[{pal['dim']}]{RECEIPT_FIELD_LABELS['coverage'].upper():<{col}}"
                f"{RECEIPT_FIELD_LABELS['checks'].upper()}[/]\n"
                f"[{pal['ink']}]{pip(ekey, pal)} {_escape(cov_cell)}[/][{ccol}]{cval}[/]")
